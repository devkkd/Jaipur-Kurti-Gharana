  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // First, get all categories to find "Top Tunics" category
        const categoriesResponse = await fetch('/api/admin/categories');
        const categoriesData = await categoriesResponse.json();
        
        if (categoriesData.success) {
          // Find the category that matches "top-tunics"
          const topTunicsCategory = categoriesData.data.find(
            cat => cat.slug === 'top-tunics' || 
                   cat.name.toLowerCase().includes('top') ||
                   cat.name.toLowerCase().includes('tunic') ||
                   cat.slug === 'ethnic-wear' // fallback
          );
          
          if (topTunicsCategory) {
            setCategory(topTunicsCategory);
            ...
            if (subcategoriesData.success) {
              const categorySubcategories = subcategoriesData.data.filter(sub => {
                ...
                return subCategoryId === topTunicsCategory._id;
              });
              setSubcategories(categorySubcategories);
            }
            
            // Fetch products for this category
            const productsResponse = await fetch(`/api/products?categoryId=${topTunicsCategory._id}&limit=50`);
      ...
      <div className="max-w-[1440px] mx-auto px-4 md:px-12 py-10 bg-white font-sans">
        ...
        <div className="text-center mb-10">
          <p className="text-sm font-semibold tracking-widest uppercase mb-1">
            {category ? category.name : 'Casual Collection'}
          </p>
          <h1 className="text-3xl md:text-4xl font-serif font-bold tracking-tight text-gray-900 uppercase">
            {category ? category.name : 'Tops & Tunics'}
          </h1>
          <p className="text-gray-600 mt-4 max-w-2xl mx-auto">
            {category?.description || 'Discover our versatile collection of stylish tops and tunics, perfect for everyday wear and special occasions.'}
          </p>
        ...
        {filteredAndSortedProducts.length > 0 ? (
          <div className={`grid ${viewMode === 'grid' ? 'grid-cols-1 sm:grid-cols-2 lg:grid-cols-4' : 'grid-cols-1'} gap-x-6 gap-y-12`}>
            {filteredAndSortedProducts.map((product) => (
              ...
              >
                {/* Image Container */}
                <div className={`relative overflow-hidden bg-gray-100 mb-4 ${viewMode === 'list' ? 'w-64 shrink-0' : 'aspect-3/4'}`}>
                ...
                <div className={`${viewMode === 'list' ? 'flex-1' : ''}`}>
                  <div className="space-y-2">
                    ...
                    <p className="text-[11px] text-gray-500 leading-relaxed line-clamp-2">
                      {product.description || 'Stylish and comfortable tops and tunics perfect for everyday wear and special occasions.'}
                    </p>
                    ...
                    {viewMode === 'list' && (
                      <div className="mt-4 text-xs text-gray-600">
                        <div className="flex gap-4">
                          <span>Material: {product.productDetails?.material || '100% Cotton/Linen'}</span>
                          <span>Care: {product.productDetails?.productCare || 'Machine Wash'}</span>
...
export default TopsTunicsListing;
