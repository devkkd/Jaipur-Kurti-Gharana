  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // First, get all categories to find "Gown" category
        const categoriesResponse = await fetch('/api/admin/categories');
        const categoriesData = await categoriesResponse.json();
        
        if (categoriesData.success) {
          // Find the category that matches "gown"
          const gownCategory = categoriesData.data.find(
            cat => cat.slug === 'gown' || 
                   cat.name.toLowerCase().includes('gown') ||
                   cat.slug === 'ethnic-wear' // fallback
          );
          
          if (gownCategory) {
            setCategory(gownCategory);
            ...
            if (subcategoriesData.success) {
              const categorySubcategories = subcategoriesData.data.filter(sub => {
                ...
                return subCategoryId === gownCategory._id;
              });
              setSubcategories(categorySubcategories);
            }
            
            // Fetch products for this category
            const productsResponse = await fetch(`/api/products?categoryId=${gownCategory._id}&limit=50`);
      ...
      <div className="max-w-[1440px] mx-auto px-4 md:px-12 py-10 bg-white font-sans">
        ...
        <div className="text-center mb-10">
          <p className="text-sm font-semibold tracking-widest uppercase mb-1">
            {category ? category.name : 'Elegant Collection'}
          </p>
          <h1 className="text-3xl md:text-4xl font-serif font-bold tracking-tight text-gray-900 uppercase">
            {category ? category.name : 'Gowns'}
          </h1>
          <p className="text-gray-600 mt-4 max-w-2xl mx-auto">
            {category?.description || 'Discover our stunning collection of gowns, perfect for special occasions, parties, and celebrations.'}
          </p>
                ...
                <div className={`${viewMode === 'list' ? 'flex-1' : ''}`}>
                  <div className="space-y-2">
                    ...
                    <p className="text-[11px] text-gray-500 leading-relaxed line-clamp-2">
                      {product.description || 'Stunning gown featuring exquisite designs perfect for special celebrations.'}
                    </p>
                    ...
                    {viewMode === 'list' && (
                      <div className="mt-4 text-xs text-gray-600">
                        <div className="flex gap-4">
                          <span>Material: {product.productDetails?.material || 'Silk/Net/Georgette'}</span>
