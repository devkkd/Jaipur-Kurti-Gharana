  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // First, get all categories to find "Co-Ord Set" category
        const categoriesResponse = await fetch('/api/admin/categories');
        const categoriesData = await categoriesResponse.json();
        
        if (categoriesData.success) {
          // Find the category that matches "co-ord" or "coord"
          const coordCategory = categoriesData.data.find(
            cat => cat.slug === 'co-ord-set' || 
                   cat.name.toLowerCase().includes('co-ord') ||
                   cat.name.toLowerCase().includes('coord') ||
                   cat.slug === 'ethnic-wear' // fallback
          );
          
          if (coordCategory) {
            setCategory(coordCategory);
            ...
            if (subcategoriesData.success) {
              const categorySubcategories = subcategoriesData.data.filter(sub => {
                ...
                return subCategoryId === coordCategory._id;
              });
              setSubcategories(categorySubcategories);
            }
            
            // Fetch products for this category
            const productsResponse = await fetch(`/api/products?categoryId=${coordCategory._id}&limit=50`);
      ...
      <div className="max-w-[1440px] mx-auto px-4 md:px-12 py-10 bg-white font-sans">
        ...
        <div className="text-center mb-10">
          <p className="text-sm font-semibold tracking-widest uppercase mb-1">
            {category ? category.name : 'Fashionable Sets'}
          </p>
          <h1 className="text-3xl md:text-4xl font-serif font-bold tracking-tight text-gray-900 uppercase">
            {category ? category.name : 'Co-Ord Sets'}
          </h1>
          <p className="text-gray-600 mt-4 max-w-2xl mx-auto">
            {category?.description || 'Discover our trendy co-ord sets, the perfect combination of style and comfort for modern fashion enthusiasts.'}
          </p>
                    ...
                    {viewMode === 'list' && (
                      <div className="mt-4 text-xs text-gray-600">
                        <div className="flex gap-4">
                          <span>Material: {product.productDetails?.material || 'Rayon/Cotton Blend'}</span>
...
export default CoOrdSetListing;
