...
export async function POST(request) {
  try {
    ...
    if (!name || name.trim().length < 2) {
      return NextResponse.json({ error: 'Product name must be at least 2 characters long' }, { status: 400 });
    }
    if (!description || description.trim().length < 10) {
      return NextResponse.json({ error: 'Product description must be at least 10 characters long' }, { status: 400 });
    }
    if (!categoryId) {
    ...
    const productData = {
      ...
      images: { main: images.main.trim(), gallery: images.gallery || [] },
    ...
    if (error.code === 11000) {
      ...
      return NextResponse.json({ error: `A product with this ${field} already exists` }, { status: 400 });
    }
